 ...
 *
 * 1. The concept of topic can be anything that uniquely identifies a communication.
 *    It does not need to be an MQTT topic as demonstrated here.
...
void recv_message(e4client *client, const char *topic_name, const char *payload, size_t payload_len)
{
     ...
     * type.
     */
    r = e4c_unprotect_message(buf, sizeof(buf) - 1, &len, (uint8_t*)payload, payload_len,
                              topic_name, &client->store);
